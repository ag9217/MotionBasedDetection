/****************************************************************************\
	Compute OFCalcOpticFlow_CCore.cpp, Entry point of the OF_CCore. 
...
\**----------------------------------**/
    if(nlhs!=3)
        mexErrMsgTxt ("OFCalcOpticFlow_CCore::3 outputs parameters are required");
    if(nrhs==4){ //Use a new OF_Core
        OF_CCore CCore(nrhs,prhs);
        plhs[0]=CCore.get_OF_phi();
        plhs[1]=CCore.get_OF_epsilon();    
        plhs[2]=CCore.get_Distances();    
    }
    else if(nrhs==2){ //Use an existing OF_Core
        OF_CCore* CCore = (OF_CCore*)mxGetData(prhs[0]);
        CCore->set_trajectory(prhs[1]);
        plhs[0]=CCore->get_OF_phi();
        plhs[1]=CCore->get_OF_epsilon();    
        plhs[2]=CCore->get_Distances();
    }
    else{
        mexErrMsgTxt ("OFCalcOpticFlow_CCore::2 or 4 input parameters are required");
    }
}
