...
%     OFF_Channel = -OFF_Channel*1;
    ...
    ON_Filter2=NewFilter2(ON_Filtered_Delayed,Tau1_ON, Ts);
    ...
    OFF_Filter2=NewFilter2(OFF_Filtered_Delayed,Tau1_OFF, Ts);
...
%     b=[1/(1+2*1.25/Ts), 1/(1+2*1.25/Ts), 1/(1+2*1.25/Ts), 1/(1+2*1.25/Ts)];
%     a=[1, (1-2*1.25/Ts)/(1+2*1.25/Ts), 1, (1-2*1.25/Ts)/(1+2*1.25/Ts)];
    persistent ONbuffer
    if isempty(ONbuffer)
