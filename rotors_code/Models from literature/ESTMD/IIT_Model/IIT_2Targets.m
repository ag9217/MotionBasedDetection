...
%[dataGreenChannel,nFrames,~]=createMotion('road','horizontal',1);
...
dataGreenChannel = zeros([size(I) nFrames],class(I));
dataGreenChannel(:,:,1) = I;

for p = 2:nFrames
    image = imread(fileNames{p});
    image = imresize(image, [240 320]);
    dataGreenChannel(:,:,p) = image(:,:,3);
end
...
for i=1:nFrames+Delay
   ...
   if i>Delay
       ...
       imshow(LM)
       imwrite(LM, fullfile('C:\Users\Armand\OneDrive - Imperial College London\Documents\University\4th year\Modules\Master project\training_videos\office_clutter\clutter_estmd_jpgs', fileNames{i-1}))
...
% hold on
%imshow(FM)
directory1=['C:\Users\Armand\OneDrive - Imperial College London\Documents\University\4th year\Modules\Master project\Models from literature\ESTMD\IIT_Model\results'];
cd(directory1)
