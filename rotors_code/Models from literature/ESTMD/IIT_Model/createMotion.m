...
%         x2_coord=[10:speed*1:300];
% %         y2_coord=30*ones(1,length(x2_coord));
%         x3_coord=[10:speed*1:300];
%         y3_coord=105*ones(1,length(x3_coord));
%     case 'horizontal'
...
for IN=1:length(x3_coord)
     Frame = insertShape(BG,'FilledCircle',[x_coord(IN) y_coord(IN) 10],'Color', 'red','Opacity',1);
     Frame = insertShape(Frame,'FilledCircle',[x1_coord(IN) y1_coord(IN) 10],'Color', 'red','Opacity',1);
     Frame = insertShape(Frame,'FilledCircle',[x2_coord(IN) y2_coord(IN) 10],'Color', 'red','Opacity',1);
     Frame = insertShape(Frame,'FilledCircle',[x3_coord(IN) y3_coord(IN) 10],'Color', 'red','Opacity',1);
%      Frame = insertShape(BG,'FilledRectangle',[x2_coord(IN) y2_coord(IN) 60, 20],'Color', 'red','Opacity',1);
%      Frame = insertShape(BG,'FilledRectangle',[x3_coord(IN) y3_coord(IN) 45, 45],'Color', 'red','Opacity',1);
...
% nFrames = 150;
groundTruth(1,:)=x3_coord;
groundTruth(2,:)=y3_coord;